...
- human-input 处理方式必须可执行；`.ai/human-input-needed.md` 必须保留稳定章节、scan follow-up 具体回答示例、`review-human-input --interaction-id <id> --decision resolved|reopened` 显式治理命令、不自动关闭追问、不把补充伪装成已验证 evidence，以及不执行 Runtime / 不创建 `.ai/task-runs` 的边界。缺失时 `content:human-confirmation` 必须失败并保留具体 `missing` detail，例如缺失章节、缺失 scan follow-up 示例或缺失 review 命令。
