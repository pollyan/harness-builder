...
- `init-summary.md` 的成熟度章节、Benchmark readiness、下一步入口和 Runtime 边界。
- guide 必需章节。
- sensor 必需章节。
- skill 模板复制和引用。
...
- scan report 缺少 evidence / LLM evidence expansion / coverage / stack validation / warning / risk / command confidence 审计信息时能通过 `content:scan-report` 报告。
