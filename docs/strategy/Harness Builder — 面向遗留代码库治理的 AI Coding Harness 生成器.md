...
North Star 描述的是目标态产品方向，但不同能力进入产品的阶段不同。为避免把远期 Runtime 能力误实现为当前 Builder CLI 职责，需要区分以下边界：

| 层级 | 近期 Harness Builder 应直接承担 | 远期或承载 Runtime 负责 |
|---|---|---|
| 项目级资产生成 | 扫描代码库，生成 Guides、Sensors、Workflow 规范、成熟度报告、改进候选和 review-only 资产 | 在 IDE / CLI / CI 中消费这些资产并执行任务 |
| Runtime 协议 | 定义任务级运行契约、Harness Mapping、Sensor Report、Decision Log、Handoff Summary 等结构 | 真实执行 Workflow、运行 Sensors、生成 `.ai/task-runs` 和任务事件流 |
| 经验与自改进 | 基于已有资产、成熟度评估、review 结果和未来 Runtime 数据生成可审查候选 | 将真实任务轨迹、Sensor 结果和人工决策持续回灌 |
| 产品化体验 | 提供向导式初始化、评估、改进和审查体验 | IDE / Dashboard / 组织级治理视图等更高层产品形态 |

因此，本文中的 Runtime、task-runs、Workflow Event Store、Dashboard 和组织级能力代表目标态协作生态，不意味着当前 Harness Builder 必须内置完整任务执行器。

### 3.7 用户旅程与向导式体验原则

Harness Builder 的直接用户是 Harness Maintainer。产品体验应像一个以成熟度框架为主线的多步骤向导，持续解释扫描发现、提出补齐建议、展示候选资产、引导用户确认边界，并帮助用户逐步建立和演进 Harness，而不是一次性抛出底层配置文件让用户自行理解。

成熟度框架是用户旅程的解释主干。Harness Builder 不只是告诉用户“扫描到了什么”，还要把扫描结果、候选资产、缺口建议和后续改进放到 L0-L4 成熟度体系中解释，让用户理解当前处在哪个阶段、为什么处在这个阶段、哪些证据支撑评分、补齐哪些能力可以进入下一阶段。

目标用户旅程可以分为五个阶段：

| 阶段 | 用户目标 | Harness Builder 应提供的体验 |
|---|---|---|
| 初次进入 | 理解这个仓库当前 AI Coding 成熟度和控制能力 | 围绕成熟度维度解释技术栈、模块、已有文档、验证命令、风险线索、不确定项和初始评分依据 |
| 建立基线 | 生成能支撑当前阶段并指向下一阶段的第一版 Harness | 按成熟度缺口引导补充团队规范、架构约束、风险区域、测试策略，并展示 Guides / Sensors / Workflow 候选 |
| 审查接管 | 判断哪些建议可信、哪些需要调整 | 区分 confirmed / candidate / review-only，说明证据、置信度、适用范围、风险和对成熟度提升的影响 |
| 运行评估 | 了解 Harness 成熟度、阻断项和改进路径 | 输出成熟度等级、维度评分、阻断原因、证据包和下一步补齐建议 |
| 持续演进 | 从任务经验和评审反馈中提升成熟度 | 生成 pending improvements、asset candidates 和 self-improve package，说明它们对应的成熟度缺口，并支持审核、采纳、废弃和回滚 |

用户体验原则：

- 默认使用中文、解释性、人类可读的表达方式，避免把内部字段名直接暴露为主要交互语言。
- 每一步都应说明“发现了什么、依据是什么、为什么建议这样做、用户可以如何调整”。
- 对低置信度或高影响判断，不应只给 yes / no，而应允许用户补充、修正或暂缓。
- 在写入正式资产前，应展示摘要、候选清单、风险和待确认项。
- 对正式 Harness 变更保持克制：高风险或不确定内容先进入候选 / review-only 状态。

向导式交互设计原则：

1. `init` 是主入口。用户第一次进入和后续维护都应优先从 `init` 开始；首次 `init` 建立 Harness 基线，再次 `init` 应识别已有 `.ai` 并进入状态感知维护入口，而不是无脑覆盖。
2. 成熟度是解释主线。扫描结果、生成资产、候选建议和下一步行动，都应围绕 L0-L4 成熟度框架解释。
3. 向导组织底层能力。`assess`、`improve`、`benchmark`、`recommend-workflow`、`self-improve` 等命令是专家和自动化能力，也可以被向导内部编排；普通用户不应被要求记住命令顺序。
...
| 类别 | 关注问题 |
|---|---|
| 可控性 | 高风险任务是否被正确识别、升级和约束 |
| 验证性 | 关键 lint / test / build / security / architecture 检查是否被发现、绑定和执行 |
| 审计性 | 规则来源、候选变更、执行决策和验证结果是否可追溯 |
| 演进性 | 成熟度缺口、pending improvements 和经验反哺是否形成闭环 |
| 接管体验 | Harness Maintainer 是否能理解、审查、修改并长期维护 Harness |
...
| 能力层级 | 包含能力 | 产品含义 |
|---|---|---|
| 基础控制层 | Scanner、Guides、Sensors、Workflow Runtime Specification | 先让项目具备可读规则、可执行验证和稳定任务协议 |
| 改进闭环层 | Maturity、Experience、Self-Improve、Recommendation | 再让 Harness 能评估缺口、沉淀经验、生成可审查改进候选 |
| 产品化扩展层 | Dashboard、组织级 Pattern Library、跨项目复用、趋势分析 | 最后支持团队级和组织级治理、复用和长期运营 |

这不是固定排期，但用于说明能力依赖：没有可靠的扫描、资产契约和验证基线，Experience 与 Self-Improve 容易生成不可审查或不可执行的建议；没有成熟度和经验闭环，Core Harness 又会停留在一次性初始化产物。
...
```text
LLM 负责理解、判断、推荐、候选生成和语义评审；
确定性程序负责 evidence 收集、schema 校验、调和、落盘、审计和显式失败。
```

LLM 不应直接无约束写入正式 Harness 资产。所有机器消费结果都应进入稳定 schema，并由确定性程序校验、调和和记录证据。证据不足、模型输出异常或结构不合法时，应明确暴露问题，而不是静默生成看似成功的资产。
...
Self-Improve 的产品原则是“生成可审查改进候选”，而不是让系统无约束自改正式规则。它应提升 Harness 质量、降低重复错误和补齐控制缺口，但不能牺牲可追溯性、审核状态和团队接管能力。
...
成熟度等级表达的是 Harness 控制能力的语义跃迁，而不是单纯文件数量：

| 等级 | 语义边界 |
|---|---|
| L0 | 主要依赖临时 prompt 和个人经验，没有稳定 Harness |
| L1 | 有规则和文档可读，但验证、执行和审计尚未进入稳定闭环 |
| L2 | 有可执行 Sensors，能发现和运行部分验证，但尚未稳定绑定任务 Workflow |
| L3 | Guides、Sensors 和 Workflow 形成项目级执行协议，关键门禁不可随意跳过 |
| L4 | 能根据任务类型、风险、历史经验和成熟度缺口持续生成可审查改进候选 |
...
上述结构是目标态资产地图。具体由谁生成、何时生成，需要按资产层级区分：

| 资产层级 | 说明 | 示例 |
|---|---|---|
| Builder 直接生成的项目级资产 | 用于初始化、评估和演进项目级 Harness | `harness-config.yaml`、Guides、Sensors、Workflow Skill、`maturity-report.md` |
| 候选 / review-only 资产 | 用于审查和确认，不直接污染正式 Harness | `pending-improvements.md`、asset candidates、self-improve package、workflow recommendation review |
| Runtime 任务级资产 | 由承载 Runtime 在真实任务执行时生成，Builder 定义契约和消费方式 | `.ai/task-runs/<task-id>/harness-map.yaml`、`sensor-report.yaml`、`decision-log.md` |
| 组织级 / 平台级资产 | 支持跨项目治理、复用和趋势分析的远期能力 | Pattern Library、组织级 maturity dashboard、跨项目经验库 |

这种分层用于保持正式资产、候选资产、任务运行数据和组织级资产的边界清晰。
